...
public class ExpressBsp {
     ...
     * @return
     */
    public ExperssResult sfExpressService(Order order, String province, String city, String ara, String street,
            String phone, String postcode, String j_contact) {

        ConnectorOP op = dao.fetch(ConnectorOP.class, Cnd.where("orderId", "=", order.getOrderId()));
        Product product = dao.fetch(Product.class, Cnd.where("productId", "=", op.getProductId()));
        ...
        exOrder.setD_tel(address.getCellphone());
        ...
        if (OrderConstant.SF_OK.equals(res.getHead())) {// 成功
            ...
            orderRes = orderRes == null ? res.getBody().getOrderSearchResponse() : orderRes;
            ...
            result.setMsg(String.format("顺丰下单失败错误代码：%s, %s", error.getCode(), error.getValue()));
        }
    ...
    public List<ExpressRoute> queryOrderRoute(@Param("orderId") String orderId) {
        ...
        List<ExpressRoute> list = dao.query(ExpressRoute.class, Cnd.where("orderId", "=", orderId));
    ...
    public List<ExpressRoute> queryRouteSearch(String orderId) {
        ...
        if (routeResponse != null && routeResponse.getRoute() != null && routeResponse.getRoute().size() > 0) {
    ...
    public ExperssResult orderSearch(String OrderId) {
        ...
        if (OrderConstant.SF_OK.equals(res.getHead())) {// 成功
            ...
            Error error = res.getERROR();
            result.setMsg(String.format("顺丰查询失败错误代码：%s, %s", error.getCode(), error.getValue()));
        }
        return result;
    }
