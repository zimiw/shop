...
/**
 * a matcher for ant pattern path.
 * 
...
public class AntPathMatcher {
    ...
    protected boolean doMatch(String pattern, String path, boolean fullMatch,
        ...
        String[] pattDirs = StringPathUtils.tokenizeToStringArray(pattern, this.pathSeparator);
        String[] pathDirs = StringPathUtils.tokenizeToStringArray(path, this.pathSeparator);
        int pattIdxStart = 0;
        ...
        if (pathIdxStart > pathIdxEnd) {
            ...
            for (int i = pattIdxStart; i <= pattIdxEnd; i++) {
                if (!pattDirs[i].equals(dirMatcher)) {
                    return false;
                }
            }
            return true;
            ...
            return false;
        } else if (!fullMatch && dirMatcher.equals(pattDirs[pattIdxStart])) {
    ...
    public String extractPathWithinPattern(String pattern, String path) {
        String[] patternParts = StringPathUtils.tokenizeToStringArray(pattern, this.pathSeparator);
        String[] pathParts = StringPathUtils.tokenizeToStringArray(path, this.pathSeparator);
    ...
    public Map<String, String> extractUriTemplateVariables(String pattern, String path) {
        ...
        StringPathUtils.state(result, "Pattern \"" + pattern + "\" is not a match for \"" + path + "\"");
        return variables;
    }
     ...
     * <tr><td>/hotels</td><td>bookings</td><td>/hotels/bookings</td></tr>
     * <tr><td>/hotels/*</td><td>/bookings</td><td>/hotels/bookings</td></tr> <tr><td>/hotels/&#42;&#42;
     * </td><td>/bookings</td><td>/hotels/&#42;&#42;/bookings</td></tr>
     * <tr><td>/hotels</td><td>{hotel}</td><td>/hotels/{hotel}</td></tr>
     * <tr><td>/hotels/*</td><td>{hotel}</td><td>/hotels/{hotel}</td></tr>
     * <tr><td>/hotels/&#42;&#42;</td><td>{hotel}</td><td>/hotels/&#42;&#42;/{hotel}</td></tr>
     * <tr><td>/*.html</td><td>/hotels.html</td><td>/hotels.html</td></tr> <tr><td>/*
     * .html</td><td>/hotels</td><td>/hotels.html</td></tr>
    ...
    public String combine(String pattern1, String pattern2) {
        if (!StringPathUtils.isNotEmpty(pattern1) && !StringPathUtils.isNotEmpty(pattern2)) {
            return "";
        } else if (!StringPathUtils.isNotEmpty(pattern1)) {
            return pattern2;
        } else if (!StringPathUtils.isNotEmpty(pattern2)) {
            return pattern1;
        } else if (!pattern1.contains("{") && match(pattern1, pattern2)) {
            return pattern2;
        } else if (pattern1.endsWith("/*")) {
            if (pattern2.startsWith(pathSeparator)) {
            ...
            if (dotPos1 == -1) {
                ...
                if (pattern1.endsWith(pathSeparator) || pattern2.startsWith(pathSeparator)) {
                    return pattern1 + pattern2;
                } else {
                    return pattern1 + pathSeparator + pattern2;
                }
     ...
     * java.util.Comparator) sort} a list so that more specific patterns (without uri templates or wild cards) come
     * before
     * generic patterns. So given a list with the following patterns: <ol> <li><code>/hotels/new</code></li>
     * <li><code>/hotels/{hotel}</code></li> <li><code>/hotels/*</code></li> </ol> the returned comparator will sort
     * this
     * list so that the order will be as indicated.
     * <p>The full path given as parameter is used to test for exact matches. So when the given path is {@code
     * /hotels/2},
    ...
    private static class AntPatternComparator implements Comparator<String>,Serializable {
         ...
         * 
         */
        private static final long serialVersionUID = -9027997162609780629L;
        ...
        public int compare(String pattern1, String pattern2) {
            ...
            int bracketCount1 = StringPathUtils.countOccurrencesOf(pattern1, "{");
            int bracketCount2 = StringPathUtils.countOccurrencesOf(pattern2, "{");
        ...
        private int getWildCardCount(String patternStr) {
            String pattern = patternStr;
            if (pattern.endsWith(".*")) {
                pattern = pattern.substring(0, pattern.length() - 2);
            }
            return StringPathUtils.countOccurrencesOf(pattern, "*");
        }
