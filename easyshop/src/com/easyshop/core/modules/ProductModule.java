...
public class ProductModule {
	 ...
	 * (status!=null) { condition.and("status", "=", status); } this.result=new
	 * HashMap<String,Object>(); this.result.put("total",
	 ...
	 */
	@At
	public Object getProductActivtyNum(@Param("prototypeId") int productTypeId) {
		...
		ActivityProduct ap = dao.fetch(
				...
				Cnd.where("productTypeId", "=", productTypeId)
						.and("status", "=", 1).and("beginTime", "<=", dayTime)
						.and("endTime", ">=", dayTime));
		int leftNum = 0;
		if (ap != null) {
	 ...
	 */
	@At
	public Object getProductApp(@Param("productId") int productId,
			@Param("appType") final int appType) {

		List<OrderAppraisal> appList = dao.query(OrderAppraisal.class, Cnd
				.where("productId", "=", productId)
				.and("appType", "=", appType).desc("appTime"));
		...
		for (OrderAppraisal app : appList) {
			...
			per = dao.fetch(Personal.class,
					Cnd.where("id", "=", app.getPersonalId()));
	 ...
	 */
	@At
	public Object getProductAppNum(@Param("productId") int productId,
			@Param("appType") final int appType) {
		String sqlStr = " SELECT a.appType,  (  CASE  WHEN b.num >0 THEN b.num ELSE 0  END )  num "
				...
				+ "        GROUP BY appType) b " + "ON  a.appType = b.appType ";
