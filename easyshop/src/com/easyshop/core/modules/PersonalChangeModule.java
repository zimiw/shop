...
public class PersonalChangeModule {
	 ...
	 * 前台用户发起换货<br/>
	 * 
	 * 换货申请: 换货原因 changeReason， 订单id orderId， 商品id productId， 数量 num， 换货说明
	 * remark， 换货凭证1 image1， 换货凭证2 image2
	 * 
	 * @return
	 */
	@At
	// @AdaptBy(type = UploadAdaptor.class, args = { "ioc:picUpload" })
	public Object orderChange(HttpSession session,
			@Param("..") final OrderChange orderChange,
		...
		// 换货凭证1 image1
		// 换货凭证2 image2
		orderChange.setImage1(image1);
		orderChange.setImage2(image2);
	...
	/**
	 * 用户发起退货<br/>
	 * 订单id orderId， 商品id productId， 数量 num， 金额 amount， 退款原因 changeReason， 退款说明
	 * remark， 退款凭证1 image1， 退款凭证2 image2
	 * 
	 * @throws Exception
	 * 
	 */
	@At
	// @AdaptBy(type = UploadAdaptor.class, args = { "ioc:picUpload" })
	public Object orderReturn(HttpSession session,
			@Param("..") final OrderChange orderChange, @Param("..") NutMap nm,
			@Param("image1") String image1, @Param("image2") String image2)
			throws Exception {
	 ...
	 */
	@At
	public ResultVo cancelOrderChange(HttpSession session,
			@Param("orderChangeId") String orderChangeId) {

		String userId = String.valueOf(session
				.getAttribute(OrderConstant.FRONT_USER_ID));

		final OrderChange orderChange = dao
				.fetch(OrderChange.class,
						Cnd.where("userId", "=", userId).and("id", "id",
								orderChangeId));
