...
public class RoleModule {
	...
	public Object getRoleLists() {
		...
		for (Role role : roles) {
			role.setFunctions(dao.query(
					Functions.class,
					Cnd.wrap(" id in (select functionId from connectorFR t where t.roleId ="
							+ role.getRoleId() + " )")));
		}
	...
	@At
	public Object getRoleIdNames() {
		List<Role> roles = dao.query(Role.class, null);
		return roles;
	}

	@At
	public Object addRole(@Param("rolename") String name,
			@Param("roleDescp") String descp) {
		this.result = new HashMap<String, Object>();
	...
	@At
	public Object updateRole(@Param("roleId") int ID,
			@Param("rolename") String name, @Param("roleDescp") String descp) {
		this.result = new HashMap<String, Object>();
	...
	@At
	public Object addRolePrivilege(@Param("roleId") int roleId,
			@Param("privilegeIds") String privilegeIds) {
		this.result = new HashMap<String, Object>();
		...
		// List<Functions> funs=dao.query(Functions.class,
		// Cnd.where("id","in",priIds));
	...
	public Object getRolePrivileges(@Param("userName") String userName) {
		...
		Role role = dao.fetch(Role.class,
				Cnd.where("roleId", "=", user.getRoleId()));
		List<ConnectorFR> conns = dao.query(ConnectorFR.class,
				Cnd.where("roleId", "=", user.getRoleId()));
		...
		List<Functions> funs = dao.query(Functions.class,
				Cnd.where("id", "in", priIds).asc("orderId"));
